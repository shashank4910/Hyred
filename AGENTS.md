...
| **ATS evidence-grounded engine** (hybrid facts + LLM + quote gate) | `CONTEXT.md` → `## ATS Resume Checker` → Evidence-grounded pipeline + Known Pitfalls (dictionary patches row) + `docs/context/session-log.md` → Session 30 | `ats-evidence-engine.ts`, semantic review, consistency gate, golden fixtures, PR #269 |
...
| **ATS Resume Checker (free tool + engine + scan report)** | `CONTEXT.md` → `## ATS Resume Checker` + Known Pitfalls (ATS keyword + dictionary-patch rows) + Session 30 + `AtsScanReport.tsx` + `lib/ats-evidence-engine.ts` | hybrid vs structural, report UI, JD match, public widget, Fix Studio entry |
