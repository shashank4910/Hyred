...
| **Workspace engineering skills** (`.agents/skills/`, from addyosmani/agent-skills) | `code-review-and-quality`, `security-and-hardening`, `debugging-and-error-recovery`, `performance-optimization`, `planning-and-task-breakdown`, `test-driven-development`, `doubt-driven-development` | multi-axis review before risky merges, OWASP/security audits, systematic debugging, perf work, breaking big features (e.g. Match Studio) into tasks, adversarial decision review |
| **Leadership personas** (`.agents/skills/`) | `founder-ceo`, `cto-architecture`, `product-owner`, `product-office-hours` | "what should we build next" prioritization, architecture/scale decisions (10/100/1000-user cost), PRDs + acceptance criteria for new features, one-question-at-a-time idea interrogation before any spec |
...
| **Ready-to-Apply / fit check + keyword chips** | `CONTEXT.md` → Known Pitfalls (two-extractions row, bullet-first row) + `lib/match-studio.ts` (unified cached extraction) + `app/(app)/jobs/[id]/ReadyToApply.tsx` + `KeywordManager.tsx` (3 tooltip tiers) + migration **0025** + Sessions **46–47** | fit check scores, keyword chip buckets/tooltips, jd_requirements cache, smart keyword injection, mismatched chips vs fit check |
