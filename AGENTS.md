...
3. **Git workflow:** new branch → PR → merge. **Never** push follow-up commits to a merged/closed PR's branch — branch off latest `main` and open a NEW PR. After merging, verify the change is live via `https://raw.githubusercontent.com/shashank4910/JobRadar/main/<path>` (local git cache can be stale in sandboxes).
