...
| **Two keyword extractions disagreed** (Session 47) | Chips (uncached `extractJdKeywordsTyped`, ran on EVERY job page view) and the fit check (cached requirements) produced different keyword universes for the same JD — chips missed JVM/JProfiler that the JD contains. | ONE extraction per job: `extractJobAnalysis()` in `lib/match-studio.ts` returns requirements + full typed keyword list together, cached in `jd_requirements` (v2 shape). Both `GET /api/match/[id]/resume` and the studio route read the same cache. Never reintroduce a second extractor for the same job. |
