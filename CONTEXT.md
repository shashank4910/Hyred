...
| Resume PDF contact block stacked as a paragraph / used a non-ASCII bullet (session 11, PR #71) | Contact details (email/phone/location/linkedin) rendered as a vertical block in older builds, and the single-line version joined them with a literal `•` glyph — non-ASCII, violating the generator's own ASCII-only design rule (#4). | `lib/pdf-resume.ts` renders contacts on ONE line with ASCII `" \| "` separators; font auto-shrinks (8.8→7.2pt floor) to fit before any wrap. **ATS review note (evidence-based):** extracting text from the generated PDF yields every field cleanly (incl. all contacts inside the navy band) + real `- ` bullets → the PDF is true selectable text, single-column, Helvetica, standard headings = no ATS blockers. The `-- N of M --` lines in text extraction are the extractor's page markers, NOT content in the PDF. |
