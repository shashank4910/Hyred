...
**Last updated:** May 30, 2026 (session 8: hardened Phase 1 multi-user auth — fixed the duplicate-email sign-in crash (read-then-insert TOCTOU race → **atomic upsert** in `lib/current-user.ts`) and the `ON DELETE CASCADE` data-loss footgun via migration **0006** (FK → `ON DELETE SET NULL` + lowercase-email trigger). Two new `## Known Pitfalls` rows added; full narrative in `docs/context/session-log.md`. PRs #56 + #58 (fix), #61 (migration), #57 + #59 (docs).)
