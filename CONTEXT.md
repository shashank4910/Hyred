...
- Move embeddings from `JSONB` to **`pgvector`** with an index once `jobs` exceeds a few thousand (cosine-in-JS won't scale). **✅ Started (Session 42, migration 0024):** `jobs.embedding_vec vector(1536)` + `candidate_jobs` RPC (DB-side cosine, newest-800 pool). Still TODO: HNSW index + similarity-ordered top-K (instead of newest-first limit-800).
...
lib/match-stats.ts         ← Dashboard counts + city list + freshness (`jobFreshnessOrFilter`, `includeExpiredJobs`, `expired=1` / Session 32). Session 41–42: `parseMinScore()` shared min parser; getDashboardCounts/listMatchCities call migration-**0024** RPCs (`dashboard_status_counts`, `match_city_labels`) with legacy fallback when the functions are missing
lib/match-studio.ts        ← (Session 46-47) Ready-to-Apply engine: `extractJobAnalysis()` = ONE cached per-job extraction (jd_requirements v2: requirements 8-16 weighted + 18-25 typed keywords) shared by fit check AND tailoring chips; evidence grading (proven deterministic / 'met' paraphrase / inferred adjacent / absent), recruiter verdict, deterministic robot score (soft skills half weight), smart preselection (cap 8). `readJobKeywordsIfCached()` (Session 53: cache-read-only) feeds `GET /api/match/[id]/resume`; the studio POST is the sole extractor.
...
lib/matcher.ts             ← Cosine similarity + embedding text builder (JS fallback path; primary path is now DB-side via migration 0024 `candidate_jobs`)
...
| **Migrations are applied manually and SELECTIVELY in prod** (Session 43) | Migration 0023 crashed with `42P01: relation "company_catalog_requests" does not exist` — migration **0017 had never been run** in prod (dream catalog tables + 3 dream_companies columns missing). Supabase SQL editor runs a script as ONE transaction, so a mid-script failure rolls back everything (no partial state). | Never assume a migration ran. Index/DDL migrations must be defensive: check `to_regclass()` before indexing a table (see 0023's `create_index_if_table_exists` helper). When a 42P01 appears, find the missing base migration with `information_schema.tables` / `to_regclass('public.table')` and run it first. |
| **Big one-shot UPDATEs hit the Supabase SQL editor upstream timeout** (Session 43) | Migration 0024's whole-table vector backfill (~25k rows of 1536-float jsonb casts) exceeded the editor's gateway timeout → whole script rolled back. | Batch heavy backfills: `update … where id in (select id … limit 5000)` re-run until 0 rows. Split migrations so cheap DDL (functions/columns) is runnable separately from the heavy data copy. |
| **Rejects were re-scored forever** (Session 40, migration **0022**) | "Already scored" was derived from `matches`, but a match row only exists at `finalScore >= minScore` — below-threshold jobs (majority of funnel) were re-scored on all 4 daily scans per profile. Biggest LLM cost multiplier. | `job_scores(profile_id, job_id, score, …)` ledger records EVERY evaluation; `alreadyScored` = matches ∪ ledger. `clearMatchesForResumeChange` deletes the matching ledger rows so a new resume re-scores. Both read/write paths tolerate pre-migration (42P01 check). |
| **Two keyword extractions disagreed** (Session 47, hardened 48) | Chips (uncached `extractJdKeywordsTyped`, ran on EVERY job page view) and the fit check (cached requirements) produced different keyword universes for the same JD — chips missed JVM/JProfiler that the JD contains. | ONE extraction per job: `extractJobAnalysis()` in `lib/match-studio.ts` returns requirements + typed keyword list, cached in `jd_requirements` (v4 shape). **Session 53: only the studio POST extracts (single writer); `GET /api/match/[id]/resume` is cache-READ-ONLY via `readJobKeywordsIfCached`** (falls back to job tags on miss). Two concurrent extractors for one job = Postgres upsert deadlock → cache never persists → re-extract every view (stacked 4 LLM calls in one studio request → 504). Never reintroduce a second extractor for the same job. |
| **JD truncation silently deleted the JD's tail** (Session 48) | The extractor sliced JDs to 5000 chars; long postings (~10k) lost their END sections — exactly where "Preferred Skills" and tool example lists ("JMeter, Gatling, k6") live. The AI never saw JMeter; nobody noticed because extraction "succeeded". | Never slice a JD below **12000 chars** for any AI prompt. Watch for this in every new JD-consuming call (scoreJob's 4000-char cap is for COST on repeated per-job calls, not one-shot cached analysis). |
| **Unranked keyword prompts pick domain wallpaper over tools** (Session 48) | "List the important keywords" with a cap let the model spend slots on "financial services"/"payment processing" and drop JMeter/Java — the tokens recruiters actually type into an ATS. Two cap raises didn't help (judgment problem, not memory problem). | Phased prompt: tool census FIRST (every named tool anywhere, example lists = one entry each, never trimmed) → skills ranked by must-have > repetition > centrality → trim domain phrases first, never tools (max 3 domain, 0 soft). PLUS deterministic post-check: every keyword must appear verbatim in the JD; tools lead the list structurally. When an LLM repeats a mistake class, add a post-check — don't raise limits. |
| **Resume keyword placement = skills-list stuffing** (Session 47) | Owner directive: keywords must be injected INTO experience bullets smartly, not dumped under "key skills". | `generateAtsResume` + `weaveKeywordsIntoResume` prompts enforce priority: (1) rewrite the most relevant experience bullet (minimal truthful edit), (2) summary/achievements, (3) TECHNICAL SKILLS last resort; never pile keywords into one sentence. Deterministic skills-append remains only as the final guarantee. |
| **Stale embeddings after JD enrichment** (Session 40) | Jobs embedded at ingest from ~500-char Adzuna stubs; `ensureFullDescription` later upgraded the JD but never re-embedded → cosine ranking permanently ranked on truncated text while the LLM scored the full JD. | `reembedJob()` in `lib/jd-fetcher.ts` re-embeds after a description upgrade (writes jsonb + vector columns). |
| **Repeated exact COUNTs + fat payloads on hot paths** (Sessions 41–42, migrations **0023/0024**) | Dashboard fired ~10 exact COUNTs per view; city dropdown fetched 1000 full rows; `/api/matches` re-counted on every scroll page; ingest shipped ~800×1536 floats (~6 MB jsonb) per scan; `matches.created_at` sort and all `ilike` filters had no usable index. | Migration 0023: 16 indexes (trigram GIN for ilike, `matches(profile_id, created_at desc)`, partial embed-queue, FK-support) + 4 redundant drops. Migration 0024: `dashboard_status_counts` / `match_city_labels` / `candidate_jobs` RPCs with legacy fallback in code. Count only on `/api/matches` page 1. |
